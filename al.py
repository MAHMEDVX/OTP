...
# sys.stdout = NullWriter()  # أزل التعليق لإخفاء المخرجات
...
if not os.path.exists(JSON_DIR):
    ...
    print(f"✅ Created {JSON_DIR} directory")
...
# ======================
def load_users():
    """تحميل جميع المستخدمين من ملف JSON"""
...
def save_users(users):
    """حفظ جميع المستخدمين في ملف JSON"""
...
def get_active_users():
    """جلب المستخدمين النشطين (غير المحظورين)"""
    users = load_users()
    active = {}
...
def create_user(telegram_id, username, email, password, first_name="", last_name=""):
    ...
    # التحقق من عدم وجود الإيميل مكرر
    for uid, data in users.items():
        if data.get('email') == email:
...
USERNAME = "alone234100"
PASSWORD = "alone234100"
BOT_TOKEN = "8552862910:AAGQ0XOtVYq50lI_DIoZ2PFctKu376M5dEY"
...
def force_sub_markup():
    """إنشاء أزرار الاشتراك الإجباري مع رسالة واضحة"""
    channels = get_all_force_sub_channels(enabled_only=True)
    if not channels:
        return None
...
def send_welcome(message):
    ...
    channels = get_all_force_sub_channels(enabled_only=True)
    if channels and not force_sub_check(telegram_id):
        ...
        if markup:
            ...
            bot.send_message(
                ...
                "📌 Please follow the channel below for the bot to work correctly:\n"
                "👇 Click the button to subscribe, then press ✅ Check Subscription",
                reply_markup=markup,
                parse_mode="Markdown"
            )
        else:
    ...
    # ✅ بعد التأكد من الاشتراك، يكمل باقي الكود
    # التحقق من الحظر
    ...
    # إرسال إشعار للأدمن عند أول استخدام
...
def check_subscription(call):
    ...
    if force_sub_check(call.from_user.id):
        ...
        # حذف رسالة الاشتراك الإجباري وإرسال رسالة الترحيب
        try:
            bot.delete_message(call.message.chat.id, call.message.message_id)
        except:
            pass
        # إرسال رسالة جديدة كـ /start
        ...
        if set_admin(admin_id, True):
            ...
            for admin in ADMIN_IDS:
                try:
                    ...
                    caption += f"🆔: `{admin_id}`\n"
                    caption += f"👤: @{user.get('telegram_username', 'None')}\n"
                    caption += f"📧: `{user.get('email', 'N/A')}`\n"
                    caption += f"📅 التاريخ: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                    bot.send_message(admin, caption, parse_mode="Markdown")
                except:
                    pass
...
def admin_ban_step2(message):
    ...
    try:
        ...
        if ban_user(uid):
            ...
            email = user.get('email', 'N/A')
            username = user.get('telegram_username', 'None')
            for admin in ADMIN_IDS:
                try:
...
def admin_broadcast_user_step1(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_panel"))
    bot.edit_message_text("📨 إرسال لمستخدم محدد:\n\nأدخل معرف المستخدم (Telegram ID):", call.message.chat.id, call.message.message_id, reply_markup=markup)
...
def admin_broadcast_user_step2(message):
    ...
    try:
        ...
        bot.reply_to(message, f"📨 أرسل الرسالة للمستخدم {uid}:")
    except ValueError:
        bot.reply_to(message, "❌ معرف غير صحيح!")
        del user_states[message.from_user.id]
...
def admin_broadcast_user_step3(message):
    ...
    try:
        ...
        bot.reply_to(message, f"❌ فشل الإرسال للمستخدم {uid}")
...
def admin_full_report(call):
    ...
    try:
        ...
        for uid, user_data in users.items():
            ...
            report += f"ID: {uid} | @{user_data.get('telegram_username', 'N/A')} | الرقم: {user_data.get('assigned_number', 'N/A')} | الحالة: {status}\n"
...
def send_otp_to_user_and_group(date_str, number, sms):
    ...
    if user_id:
        try:
            ...
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("Owner ~", url="https://t.me/a7_a7l"),
                types.InlineKeyboardButton("Channel", url="https://t.me/aloneotp")
            )
    ...
    keyboard = {
        "inline_keyboard": [
            [
                ...
                {"text": "Owner 2", "url": "https://t.me/A_8M_A"}
            ],
            [
                {"text": "User Bot", "url": "https://t.me/alone_otp1bot"},
                {"text": "Channel", "url": "https://t.me/aloneotp"}
            ]
        ]
    }
