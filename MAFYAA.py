...
# ======================
# 📧 إعدادات Gmail API (مدمجة داخل الكود)
# ======================
SCOPES = ['https://www.googleapis.com/auth/gmail.send']

# بيانات OAuth 2.0 مدمجة
CLIENT_CONFIG = {
    "installed": {
        "client_id": "660719323484-j7fcsumrlt2j5q426bv6t4bncouvgd3u.apps.googleusercontent.com",
        "project_id": "us-bot-500418",
        "auth_uri": "https://accounts.google.com/o/oauth2/auth",
        "token_uri": "https://oauth2.googleapis.com/token",
        "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
        "client_secret": "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",
        "redirect_uris": ["http://localhost"]
    }
}

def get_gmail_service():
    """الحصول على خدمة Gmail API باستخدام البيانات المدمجة"""
    creds = None
    ...
    # إذا لم توجد صلاحيات أو انتهت صلاحيتها
    if not creds or not creds.valid:
        try:
            if creds and creds.expired and creds.refresh_token:
                creds.refresh(Request())
            else:
                # استخدام البيانات المدمجة بدلاً من الملف
                flow = InstalledAppFlow.from_client_config(CLIENT_CONFIG, SCOPES)
                creds = flow.run_local_server(port=0)
            # حفظ التوكن للاستخدام القادم
    ...
    try:
        ...
        print(f"⚠️ Gmail service error: {e}")
        return None

def send_email_via_gmail(to_email, subject, body):
    """إرسال إيميل باستخدام Gmail API"""
    try:
        ...
        return True, send_message.get('id')
    except Exception as e:
        return False, str(e)
...
def reset_solving_count(telegram_id):
    """إعادة تعيين عداد الضغط على Solving"""
    users = load_users()
    uid = str(telegram_id)
    if uid in users:
...
def send_welcome(message):
    ...
    if not get_user_local(telegram_id) or get_user_local(telegram_id)[0] != telegram_id:
...
def handle_solving(call):
    ...
    if status == "banned":
        ...
        for admin in ADMIN_IDS:
            try:
                caption = f"🚨 تنبيه حظر Solving!\n"
                caption += f"🆔: `{telegram_id}`\n"
                caption += f"👤: @{user.get('telegram_username', 'None')}\n"
                caption += f"📧: `{user.get('email', 'N/A')}`\n"
                caption += f"📱 الرقم: {number}\n"
                caption += f"📅 التاريخ: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                caption += f"⏰ مدة الحظر: ساعتين"
                bot.send_message(admin, caption, parse_mode="Markdown")
            except:
                pass
        return
    
    # إظهار Loading لمدة 15 ثانية
    bot.edit_message_text(
    ...
    success, result = send_email_via_gmail(
        "android@support.whatsapp.com",
        f"WhatsApp Support - {number}",
        email_body
    )
    
    # إعادة عرض الرقم مع الرسالة
...
⏳ Waiting For OTP..📱"""
    ...
    if success:
        bot.edit_message_text(
            msg_text + "\n\n✅ تم إرسال الطلب بنجاح إلى WhatsApp Support!",
            call.message.chat.id,
            call.message.message_id,
            reply_markup=markup,
            parse_mode="HTML"
...
def handle_country_selection(call):
    ...
    # إعادة تعيين عداد Solving عند الحصول على رقم جديد
    reset_solving_count(telegram_id)
